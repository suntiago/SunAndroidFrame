...
public abstract class Request<T> implements Comparable<Request<T>> {
    ...
    public final int getConnetTimeoutMs() {
        return HttpConfig.CONN_TIMEOUT;
    }

    public final int getReadTimeoutMs() {
        return (mReadTimeout > 0) ? mReadTimeout : HttpConfig.READ_TIMEOUT;
    }
                ...
                if (error.networkResponse != null) {
                    ...
                    if (cause != null) {
                        if (cause instanceof ConnectException) {
                            ...
                            errorNo = -2;
                        } else if (cause instanceof SSLProtocolException || cause instanceof SSLHandshakeException) {
                            errorNo = -3; // treat as timeout
                        } else if (cause instanceof SSLHandshakeException) {
                            errorNo = -4; // treat as timeout
                        }
                    } else {
                        errorNo = -1;
                    }
